...
(defmethod game-run ((g game))
  ...
  (sdl:with-events (:poll)
     ...
     (:mouse-motion-event (:state state :x x :y y :x-rel x-rel :y-rel y-rel)
      ;; (format t "mouse-move state(~a) x(~a) y(~a) x-rel(~a) y-rel(~a) x-sc(~a) y-sc(~a)~%" state x y x-rel y-rel (unscale-xy x-rel) (unscale-xy y-rel))
      (when (= state 1)
        (setf (game-vx g) (- (game-vx g) (unscale-xy x-rel)))
        (setf (game-vy g) (- (game-vy g) (unscale-xy y-rel)))
        (if (< (game-vx g) 0)
            (setf (game-vx g) 0)) 
        (if (>= (game-vx g) (game-width g))
            (setf (game-vx g) (- (game-width g) 1))) 
        (if (< (game-vy g) 0)
            (setf (game-vy g) 0)) 
        (if (>= (game-vy g) (game-height g))
            (setf (game-vy g) (- (game-height g) 1)))) 
      )
     (:mouse-button-down-event (:button button :x x :y y)
      ;; (format t "mouse-down button(~a) x(~a) y(~a)~%" button x y)
      )
     ;; make living cell with [MouseLeftClick]
     (:mouse-button-up-event (:button button :x x :y y)
      ;; (format t "mouse-up button(~a) x(~a) y(~a)~%" button x y)
...
(defun mouse-handler (g button x y)
  (let* ((x (truncate (+ (game-vx g) (/ x *cell-size*))))
		 (y (truncate (+ (game-vy g) (/ y *cell-size*)))))
	(case button
